...
const Masonry: React.FC<MasonryProps> = ({
  ...
  useLayoutEffect(() => {
    if (!imagesReady || !isVisible) return;
  ...
  }, [grid, imagesReady, isVisible, blurToFocus, duration, ease, getInitialPosition, stagger]);
    ...
    <div ref={containerRef} className="relative w-full" style={{ height: totalHeight }}>
      {grid.map(item => (
        <div
          ...
          style={{ willChange: 'transform, width, height, opacity', opacity: 0 }}
          onClick={() => item.url && window.open(item.url, '_blank', 'noopener')}
          onMouseEnter={e => handleMouseEnter(item.id, e.currentTarget)}
          onMouseLeave={e => handleMouseLeave(item.id, e.currentTarget)}
        >
          <div
            className="relative w-full h-full bg-cover bg-center rounded-[10px] shadow-[0px_10px_50px_-10px_rgba(0,0,0,0.4)]"
